...
# Load ConvNeXt-S Pretrained Model
class ConvNeXtSForClassification(nn.Module):
    def __init__(self, out_channels=11):  # Assuming 11 classes
        super(ConvNeXtSForClassification, self).__init__()
        self.convnext_s = models.convnext_small(pretrained=True)

        # Replace the final fully connected layer for classification
        num_ftrs = self.convnext_s.classifier[2].in_features
        self.convnext_s.classifier[2] = nn.Linear(num_ftrs, out_channels)

    def forward(self, x):
        return self.convnext_s(x)

# Instantiate Model and Optimizer
model = ConvNeXtSForClassification().to(device)
