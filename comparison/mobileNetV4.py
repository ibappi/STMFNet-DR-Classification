...
# Load MobileNetV3 Pretrained Model
class MobileNetV3ForClassification(nn.Module):
    def __init__(self, out_channels=11):  # Assuming 11 classes
        super(MobileNetV3ForClassification, self).__init__()
        self.mobilenet_v3 = models.mobilenet_v3_large(pretrained=True)

        # Replace the final fully connected layer for classification
        num_ftrs = self.mobilenet_v3.classifier[3].in_features
        self.mobilenet_v3.classifier[3] = nn.Linear(num_ftrs, out_channels)

    def forward(self, x):
        return self.mobilenet_v3(x)

# Instantiate Model and Optimizer
model = MobileNetV3ForClassification().to(device)
