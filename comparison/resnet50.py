...
class ResNet50ForClassification(nn.Module):
    def __init__(self, out_channels=11):  # Assuming 11 classes
        super(ResNet50ForClassification, self).__init__()
        self.resnet50 = models.resnet50(pretrained=True)

        # Replace the final fully connected layer for classification
        num_ftrs = self.resnet50.fc.in_features
        self.resnet50.fc = nn.Linear(num_ftrs, out_channels)

    def forward(self, x):
        return self.resnet50(x)

# Instantiate Model and Optimizer
model = ResNet50ForClassification().to(device)
